...
import { reactivateCard } from './reactivate-card';
...
describe('reactivateCard', () => {
  ...
  it('calls http with correct path, method, and token without a body', async () => {
    ...
    const result = await reactivateCard(cardId, token);

    expect(mockHttp).toHaveBeenCalledWith(
      `/api/insurance/cards/${cardId}/reactivate`,
      {
        method: 'PATCH',
        token,
      },
  ...
  it('propagates http errors', async () => {
    ...
    await expect(reactivateCard(cardId, token)).rejects.toThrow('Forbidden');
  });
});
