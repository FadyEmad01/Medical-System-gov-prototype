...
import { getLatestVerification } from './get-latest-verification';
...
describe('getLatestVerification', () => {
  ...
  it('calls http with correct path, method, and token', async () => {
    ...
    const result = await getLatestVerification(1, 'valid-jwt-token');

    expect(mockHttp).toHaveBeenCalledWith(
      '/api/insurance/verification/1/latest',
      {
        method: 'GET',
  ...
  it('propagates http errors', async () => {
    ...
    await expect(getLatestVerification(999, 'valid-jwt-token')).rejects.toThrow(
      'Not Found',
    );
  });
});
