...
describe('issueCards', () => {
  ...
  const applicationId = '11111111-1111-4111-8111-111111111112';
  ...
  it('calls http with correct path, method, and token without a body', async () => {
    ...
    const result = await issueCards(applicationId, token);

    expect(mockHttp).toHaveBeenCalledWith(
      `/api/insurance/cards/issue/${applicationId}`,
      {
        method: 'POST',
        token,
      },
  ...
  it('propagates http errors', async () => {
    ...
    await expect(issueCards(applicationId, token)).rejects.toThrow('Forbidden');
  });
});
