...
import { getPatientDocuments } from './get-patient-documents';
...
describe('getPatientDocuments', () => {
  ...
  it('calls http with correct path, method, and token without a body', async () => {
    ...
    const result = await getPatientDocuments(patientId, token);
  ...
  it('propagates http errors', async () => {
    ...
    await expect(getPatientDocuments(patientId, token)).rejects.toThrow(
      'Forbidden',
    );
  });
});
