...
import { getCurrentCard } from './get-current-card';
...
describe('getCurrentCard', () => {
  ...
  it('calls http with correct path, method, and token without a body', async () => {
    ...
    const result = await getCurrentCard(patientId, token);

    expect(mockHttp).toHaveBeenCalledWith(
      `/api/insurance/cards/current/${patientId}`,
      {
        method: 'GET',
        token,
      },
  ...
  it('propagates http errors', async () => {
    ...
    await expect(getCurrentCard(patientId, token)).rejects.toThrow('Forbidden');
  });
});
