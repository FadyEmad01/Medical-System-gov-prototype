...
describe('renewCard', () => {
  ...
  const mockResponse: CardResponse = {
    ...
    expiresAt: '2028-08-02T10:00:00Z',
  ...
  it('calls http with correct path, method, body, and token', async () => {
    ...
    const data: ChangeCardStatusRequest = { reason: 'Annual renewal' };

    const result = await renewCard(cardId, data, token);

    expect(mockHttp).toHaveBeenCalledWith(
      `/api/insurance/cards/${cardId}/renew`,
      {
        method: 'POST',
        body: data,
        token,
  ...
  it('propagates http errors', async () => {
    ...
    await expect(
      renewCard(cardId, { reason: 'Annual renewal' }, token),
