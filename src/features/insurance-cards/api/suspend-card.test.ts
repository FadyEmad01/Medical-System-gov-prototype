...
import { suspendCard } from './suspend-card';
...
describe('suspendCard', () => {
  ...
  it('calls http with correct path, method, body, and token', async () => {
    ...
    const data: ChangeCardStatusRequest = { reason: 'Suspected fraud' };

    const result = await suspendCard(cardId, data, token);

    expect(mockHttp).toHaveBeenCalledWith(
      `/api/insurance/cards/${cardId}/suspend`,
      {
        method: 'PATCH',
        body: data,
        token,
  ...
  it('propagates http errors', async () => {
    ...
    await expect(
      suspendCard(cardId, { reason: 'Suspected fraud' }, token),
